a,d = map(lambda x: int(x), input().split())


def readTimeAndElev(segments):
    hs = []
    ts = []

    for _ in range(segments):
        h, t = map(lambda x: int(x), input().split())
        hs.append(h)
        ts.append(t)

    return hs, ts

aElevation, aTime = readTimeAndElev(a)
dElevation, dTime = readTimeAndElev(d)
